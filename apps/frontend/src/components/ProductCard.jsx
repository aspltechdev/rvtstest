...
export default function ProductCard({ image, isActive = false }) {
    return (
        <div className={`relative group w-[280px] h-[400px] md:w-[320px] md:h-[450px] dark:bg-zinc-900/80 bg-white/80 backdrop-blur-sm rounded-xl border ${isActive ? 'border-brand-red shadow-[0_0_30px_rgba(208,0,0,0.2)]' : 'dark:border-zinc-800 border-zinc-200'} p-5 flex flex-col items-center justify-between transition-all duration-500 hover:scale-105 hover:border-brand-red hover:shadow-[0_0_30px_rgba(208,0,0,0.4)]`}>
            ...
            <div className="w-full h-[70px] dark:bg-black bg-white border dark:border-zinc-800 border-zinc-200 rounded-lg flex flex-col items-center justify-center gap-1 group-hover:border-brand-red dark:group-hover:bg-zinc-950 group-hover:bg-red-50 transition-all duration-300 mt-4">
                ...
                <span className="dark:text-zinc-500 text-zinc-600 group-hover:text-brand-red text-[10px] tracking-[0.2em] font-bold uppercase transition-colors">Mounting Fixed</span>
            </div>
        </div>
    )
}
